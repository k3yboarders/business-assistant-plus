...
// FUNCTIONS 
function calculatePITBasedOnIncome(income, typeOfTaxation, polskiLadVersion, middleClassReduction = false) {
    ...
    const shouldReduceByHealthcare = $('#healthcare-reduction-input').prop('checked');

    if(income <= taxFreeIncome)
        return 0;
    if(polskiLadVersion === 1 && middleClassReduction)
        income -= calculateMiddleClassReduction(income);
    if(polskiLadVersion === 2 && isNonStandardType(typeOfTaxation) && shouldReduceByHealthcare)
        income -= Math.min(calculateHealthcareTax(income, typeOfTaxation), 8700);
    switch(typeOfTaxation) {

    case 'standard':
        return income <= firstTaxThreshold? firstStandardPercentage * income - taxReducingAmount :
            secondThresholdConstAmount + secondStandardPercentage * (income - firstTaxThreshold);
    case 'line':
        return 0.19 * income;
    }
...
function changeToTaxationCard() {
    ...
    $('label[for="healthcare-reduction-input"]').text('Odliczenie składki zdrowotnej od podatku');
...
function changeToNormalMode() {
    ...
    $('label[for="pit-input"]').text('Dochód w roku podatkowym:');
    $('label[for="healthcare-reduction-input"]').text('Odliczenie składki zdrowotnej od przychodu/dochodu');
...
function isNonStandardType(taxationType) {
    return [typesOfPITTaxation.line, typesOfPITTaxation.lumpSum, typesOfPITTaxation.taxationCard]
...
$('.container').on('input', 'input[type="number"]',  function() {
    ...
    if(isNaN(income) || (income > twoMillionEuros && taxType === typesOfPITTaxation.lumpSum ))
        enterErrorState($('#pit-input'));
    else
        resetErrorState($('input[type="number"]'));

    if(!calculateMiddleClassReduction(income) || taxType !== typesOfPITTaxation.standard) {
        disableMiddleClassReductionCheckbox();
        return;
    }
    enableMiddleClassReductionCheckbox();
...
$('.confirmation-button').on('click', () => {
    ...
    if(!validateInputFields($('input[type="number"]')))
        return;
    ...
    const healthcareTax = calculateHealthcareTax(value, typeOfTaxation).toFixed(2);
    const vat = calculateVAT(parseFloat($('#sales-income').val()), parseInt($('#vat-percentages').val()) / 100).toFixed(2);
    ...
    switch(typeOfTaxation) {
    case typesOfPITTaxation.standard:
        pit = calculatePITBasedOnIncome(value, typesOfPITTaxation.standard, version, reduction).toFixed(2);
        break;
    case typesOfPITTaxation.line:
        pit = calculatePITBasedOnIncome(value, typesOfPITTaxation.line, version, reduction).toFixed(2);
        break;
    case typesOfPITTaxation.lumpSum:
    ...
    $('.pit-value-paragraph').text('PIT: ' + pit + 'zł');
    $('.healthcare-value-paragraph').text('Składka zdrowotna roczna: ' + healthcareTax + 'zł');
    $('.vat-value-paragraph').text('VAT: ' + vat + 'zł');
    $('.property-value-paragraph').text('Podatek od nieruchomości: ' + propertyTax + 'zł');
    $('.civil-legal-value-paragraph').text('Podatek od czynności cywilno-prawnych: ' + civilLegalTax + 'zł');

    const sumOfTaxes = $('.result-paragraph').length? $('.result-paragraph') : $('.pit-value-paragraph').clone();
    const numericalSum = parseFloat(pit) + parseFloat(healthcareTax) + parseFloat(vat) + parseFloat(propertyTax) + parseFloat(civilLegalTax);
    sumOfTaxes.removeClass('pit-value-paragraph');
    sumOfTaxes.addClass('result-paragraph');
    sumOfTaxes.text('Razem: ' + numericalSum.toFixed(2) + 'zł');
